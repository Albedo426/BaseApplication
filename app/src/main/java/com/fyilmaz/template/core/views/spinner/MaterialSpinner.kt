package com.fyilmaz.template.core.views.spinner
...
open class MaterialSpinner @JvmOverloads constructor(
     ...
     */
    val editText = AppCompatEditText(getContext())
    ...
    init {
        context.obtainStyledAttributes(attrs, R.styleable.MaterialSpinner).run {
            ...
            editText.background = ContextCompat.getDrawable(context, R.drawable.ic_round_edit_text)
            editText.isEnabled =
            ...
            editText.hint = getString(R.styleable.MaterialSpinner_materialHint)
            ...
            getDimensionPixelSize(
                ...
                14
            ).let { if (it > 0) editText.setTextSize(DP, it.toFloat()) }
            ...
            colorStateList = context.obtainStyledAttributes(
                attrs,
                intArrayOf(androidx.appcompat.R.attr.colorControlActivated)
            ).run {
                val activated = getColor(0, 0)
                val normal = getContext().getColorCompat(R.color.cardview_shadow_end_color)
                recycle()
                ColorStateList(
                    arrayOf(
            ...
            theme.applyStyle(R.style.MaterialSpinner, true)
